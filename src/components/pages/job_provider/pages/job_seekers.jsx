...
const Employer_Job_Seekers = () => {
  ...
  useEffect(() => {
    const fetchJobSeekers = async () => {
      try {
        ...
        console.log("Retrieved job seekers data: ", response.data);

        // Ensure we're working with an array
        const jobSeekersData = Array.isArray(response.data) ? response.data : [];
        
        // Filter active job seekers (status === true)
        const activeJobSeekers = jobSeekersData.filter(
          (seeker) => seeker.status === true
        );

        setJobSeekers(activeJobSeekers);
        setFilteredSeekers(activeJobSeekers); // Initialize filtered seekers with all active seekers
  ...
  const calculateStats = (seekers) => {
    const maleCount = seekers.filter(
      (seeker) => seeker.gender?.toLowerCase() === "male"
    ).length;
    const femaleCount = seekers.filter(
      (seeker) => seeker.gender?.toLowerCase() === "female"
    ).length;
    const totalExperience = seekers.reduce(
      (sum, seeker) => sum + (parseFloat(seeker.experience) || 0,
      0
    ));
    const avgExp =
      seekers.length > 0 ? (totalExperience / seekers.length).toFixed(1) : 0;
    const higherEduCount = seekers.filter(
      (seeker) =>
  ...
  // Apply filters whenever filters or jobSeekers change
  useEffect(() => {
    const applyFilters = () => {
      let result = [...jobSeekers]; // Start with all job seekers
      ...
      if (filters.searchQuery) {
        ...
        result = result.filter(
          (seeker) =>
            `${seeker.first_name} ${seeker.middle_name || ""} ${seeker.last_name}`
              .toLowerCase()
              .includes(query) ||
  ...
  const getInitials = (seeker) => {
    return `${seeker.first_name?.charAt(0) || ''}${seeker.last_name?.charAt(0) || ''}`.toUpperCase();
  };
  ...
  const currentSeekers = filteredSeekers.slice(
    indexOfFirstSeeker,
    indexOfLastSeeker
  );
          ...
          <>
                    ...
                    <div className="p-6">
                      <div className="flex flex-col items-center text-center mb-4">
                        ...
                        <p className="text-blue-600">
                          {seeker.education_sector || seeker.education_level || "Education not specified"}
                        </p>
                      </div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          >
                            <path
                              ...
                              strokeLinejoin="round"
                              strokeWidth="2"
                              d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 012-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"
                            />
                          </svg>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          >
                            <path
                              ...
                              strokeLinejoin="round"
                              strokeWidth="2"
                              d="M12 8v4l3 3m6-3a9 9 0 11-14 0 9 9 0 0114 0z"
                            />
                          </svg>
                          <span className="text-gray-600">
                            Salary: {formatSalary(seeker.salary_range)}
                          </span>
                        </div>
                      </div>
            ...
            <div className="flex justify-between items-center bg-white rounded-lg shadow p-4">
              <div className="text-sm text-gray-600">
                Showing {indexOfFirstSeeker + 1} to{" "}
                {Math.min(indexOfLastSeeker, filteredSeekers.length)} of{" "}
                {filteredSeekers.length} job seekers
              </div>
          ...
          <div className="relative bg-white rounded-lg shadow-xl max-w-3xl w-full max-h-screen overflow-auto">
              ...
              <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="md:w-2/3">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <div className="bg-gray-50 p-4 rounded-lg">
                      ...
                      <p className="text-gray-800">
                        {formatExperience(selectedSeeker.experience)}
                      </p>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                      ...
                      <p className="text-gray-800">
                        {selectedSeeker.gender || "Not specified"}
                      </p>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <h3 className="font-semibold text-gray-700">
                      ...
                      <p className="text-gray-800">
                        {formatSalary(selectedSeeker.salary_range)}
                      </p>
                    </div>
                  </div>
                  ...
                  {selectedSeeker.resume && (
                    <div className="mt-4">
                      <h3 className="font-semibold text-lg mb-2 text-gray-700">Resume</h3>
                      <a 
                        href={`https://anaweza-backend.up.railway.app${selectedSeeker.resume}`}
                        target="_blank" 
            ...
            <div className="bg-gray-50 px-6 py-4 rounded-b-lg">
              <div className="flex justify-end gap-4">
                <button
                  onClick={() =>
                    navigate(`/employer/chat/${selectedSeeker.id}`)
                  }
                  className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700"
                >
                  Chat with {selectedSeeker.first_name}
                </button>
                <button
                  onClick={closeModal}
