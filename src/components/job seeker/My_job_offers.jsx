...
const Job_Seeker_JobManagement = () => {
              ...
              filteredJobs.map(job => (
                <tr key={job.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    ...
                    <button onClick={() => handleViewDetails(job.id)} className="text-blue-600 hover:text-blue-900">
                        <Eye size={18} />
                      </button>
                      <button onClick={() => handleEdit(job)} className="text-blue-600 hover:text-blue-900">
                        <Edit size={18} />
                      </button>
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto p-6">
          ...
          <div className="space-y-6">
            ...
            <div className="border-b pb-4">
              <div className="flex justify-between items-start">
                ...
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusBadge(job.status)}`}>
                  {job.status === 'active' && <CheckCircle2 size={14} className="mr-1" />}
                  {job.status === 'closed' && <AlertCircle size={14} className="mr-1" />}
                  {t(job.status)}
                </span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                ...
                <div>
                  <span className="font-medium text-gray-700">{t("Offer Type")}:</span>
                  <span className="ml-2 text-gray-600">{t(job.offer_type)}</span>
                </div>
                <div>
                  <span className="font-medium text-gray-700">{t("Experience Level")}:</span>
                  <span className="ml-2 text-gray-600">{t(job.experience_level)}</span>
                </div>
                {job.job_type && (
                  <div>
                    <span className="font-medium text-gray-700">{t("Job Type")}:</span>
                    <span className="ml-2 text-gray-600">{job.job_type.name}</span>
                  </div>
                )}
                {job.job_category && (
                  <div>
                    <span className="font-medium text-gray-700">{t("Category")}:</span>
                    <span className="ml-2 text-gray-600">{job.job_category.name}</span>
                  </div>
                )}
                {job.salary_range && (
                  <div>
                    <span className="font-medium text-gray-700">{t("Salary Range")}:</span>
                    <span className="ml-2 text-gray-600">{job.salary_range}</span>
                  </div>
                )}
                <div>
                  <span className="font-medium text-gray-700">{t("Application Deadline")}:</span>
            ...
            {/* Description Section */}
            <div>
              <h4 className="text-lg font-semibold text-gray-800 mb-2">{t("Description")}</h4>
            ...
            {job.requirements && job.requirements.length > 0 && (
              <div>
                <h4 className="text-lg font-semibold text-gray-800 mb-2">{t("Requirements")}</h4>
                <ul className="list-disc pl-5 space-y-1">
                  {Array.isArray(job.requirements) ? (
                    ...
                    <li className="text-gray-600">{job.requirements}</li>
                  )}
                </ul>
              </div>
            )}
            ...
            {job.responsibilities && job.responsibilities.length > 0 && (
              <div>
                <h4 className="text-lg font-semibold text-gray-800 mb-2">{t("Responsibilities")}</h4>
                <ul className="list-disc pl-5 space-y-1">
                  {Array.isArray(job.responsibilities) ? (
                    ...
                    <li className="text-gray-600">{job.responsibilities}</li>
                  )}
                </ul>
              </div>
            )}
...
<div>
              <h4 className="text-lg font-semibold text-gray-800 mb-2">{t("Number of position")}</h4>
...
export default Job_Seeker_JobManagement;